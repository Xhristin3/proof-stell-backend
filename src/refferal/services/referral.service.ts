...
export class ReferralService {
  ...
  async createReferralRelationship(
    ...
    try {
      ...
      await this.awardRegistrationBonus(
        referee,
        savedReferral,
        queryRunner.manager,
      );
      ...
      this.logger.log(
        `Referral relationship created: ${referrer.id} -> ${referee.id}`,
      );
  ...
  private async awardRegistrationBonus(
    referee: User,
    referral: Referral,
    manager: any,
  ): Promise<void> {
  ...
  async handleRegistrationComplete(userId: string): Promise<void> {
    ...
    try {
      ...
      if (referral) {
        ...
        this.logger.log(
          `Registration reward awarded to referrer: ${referral.referrerId}`,
        );
      }
  ...
  async handleFirstGameComplete(userId: string): Promise<void> {
    ...
    try {
      ...
      if (referral) {
        ...
        this.logger.log(
          `Milestone reward awarded to referrer: ${referral.referrerId}`,
        );
      }
