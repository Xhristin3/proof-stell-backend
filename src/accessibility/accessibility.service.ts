...
function wcagMeta(
  criterion: string,
): Record<'wcagCriterion' | 'wcagLevel' | 'wcagReference', string> {
  const guideline = WCAG_GUIDELINES[criterion];
  if (!guideline)
    return { wcagCriterion: '', wcagLevel: '', wcagReference: '' };
  return {
...
export class AccessibilityService {
  ...
  validateImageAltText(
    ...
    if (image.mimeType && !ALLOWED_IMAGE_MIME_TYPES.has(image.mimeType)) {
      issues.push({
        ...
        suggestion:
          'Use a standard web image format such as JPEG, PNG, WebP, or SVG.',
      });
    }
    ...
    if (image.isDecorative) {
      if (image.altText !== '') {
        issues.push({
          ...
          message:
            "Decorative images must have an empty alt attribute (alt='') so screen readers skip them.",
          path: `${path}.altText`,
          severity: 'medium',
          context: { url: image.url, altText: image.altText },
          ...wcagMeta('1.1.1'),
          suggestion:
            'Set altText to an empty string ("") for decorative images.',
        });
      }
    ...
    if (altText === undefined || altText === null) {
      issues.push({
        ...
        ...wcagMeta('1.1.1'),
        suggestion:
          'Add a descriptive alt attribute, or set isDecorative=true for presentational images.',
      });
      return issues;
    }
    ...
    if (trimmed === '') {
      issues.push({
        code: 'EMPTY_ALT_TEXT',
        message:
          'Image has an empty alt attribute but is not marked as decorative.',
        path: `${path}.altText`,
        severity: 'high',
        context: { url: image.url },
        ...wcagMeta('1.1.1'),
        suggestion:
          'Either provide a meaningful description or set isDecorative=true.',
      });
      return issues;
    }
    ...
    if (trimmed.length < ALT_TEXT_MIN_LENGTH) {
      issues.push({
        ...
        ...wcagMeta('1.1.1'),
        suggestion:
          'Provide a concise but descriptive alt text of at least 5 characters.',
      });
    }
    ...
    if (trimmed.length > ALT_TEXT_MAX_LENGTH) {
      issues.push({
        ...
        ...wcagMeta('1.1.1'),
        suggestion:
          'Keep alt text concise. For complex images, use a visible caption or longdesc.',
      });
    }
    ...
    if (isNonDescriptiveAltText(trimmed)) {
      issues.push({
        ...
        message:
          'Alt text appears to be auto-generated from a filename or is a generic placeholder.',
        path: `${path}.altText`,
        severity: 'medium',
        context: { url: image.url, altText: trimmed },
        ...wcagMeta('1.1.1'),
        suggestion:
          'Replace the alt text with a description of what the image depicts.',
      });
    }
    ...
    if (!image.width || !image.height) {
      issues.push({
        ...
        message:
          'Image is missing width or height attributes, which can cause layout shift.',
        path: `${path}`,
        severity: 'info',
        context: { url: image.url },
        suggestion:
    ...
    if (content.type === 'heading') {
      if (!content.level || content.level < 1 || content.level > 6) {
        issues.push({
          ...
          ...wcagMeta('2.4.6'),
          suggestion:
            'Set the heading level to a number between 1 (most important) and 6.',
        });
      }
    }
    ...
    if (content.type === 'button' || content.type === 'link') {
      ...
      if (content.type === 'link' && !content.content.trim() && !hasAriaLabel) {
        issues.push({
          code: 'EMPTY_LINK',
          message:
            'Link element has no descriptive text, making it unusable by screen reader users.',
          path: `${path}.content`,
          severity: 'high',
          ...wcagMeta('4.1.2'),
          suggestion:
            'Provide descriptive link text that conveys the destination or action.',
        });
      }
    }
    ...
    if (content.type === 'input') {
      ...
      if (!hasLabel) {
        issues.push({
          ...
          ...wcagMeta('4.1.2'),
          suggestion:
            'Associate a <label> element via labelledBy, or add an aria-label attribute.',
        });
      }
    }
    ...
    if (content.lang !== undefined) {
      ...
      if (!langPattern.test(content.lang)) {
        issues.push({
          ...
          suggestion:
            "Use a valid BCP 47 language tag, e.g. 'en', 'fr', or 'pt-BR'.",
        });
      }
    }
  ...
  processAccessibilityReport(report: AccessibilityReport): ValidationResult {
    ...
    if (report.score !== undefined) {
      const level =
        report.score >= 90 ? 'log' : report.score >= 70 ? 'warn' : 'error';
  ...
  getWCAGGuidelinesSummary(): string {
    ...
    for (const g of Object.values(WCAG_GUIDELINES)) {
      lines.push(
        `  ${g.criterion} [${g.level}] — ${g.title}: ${g.description}`,
      );
