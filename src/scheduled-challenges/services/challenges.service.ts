...
export class ChallengesService {
  ...
  async submitChallenge(
    ...
    if (!attempt) {
      throw new NotFoundException(
        `Challenge attempt with ID ${attemptId} not found`,
      );
    }
    ...
    await this.dynamicDifficultyService.updateUserDifficulty(
      userId,
  ...
  private calculateScore(
    ...
    const timeBonus = Math.max(
      0,
      ((challenge.timeLimit - timeSpent) / challenge.timeLimit) * 0.2,
    );
