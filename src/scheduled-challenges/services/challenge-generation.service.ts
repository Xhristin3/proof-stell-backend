...
export class ChallengeGenerationService {
  ...
  async generateChallengeForUser(
    userId: string,
    challengeType: ChallengeType,
  ): Promise<Challenge> {
    const userDifficulty =
      await this.dynamicDifficultyService.getUserDifficulty(
        userId,
        challengeType,
      );
  ...
  private async createDynamicChallenge(
    type: ChallengeType,
    difficulty: DifficultyLevel,
  ): Promise<Challenge> {
    ...
    this.logger.log(
      `Generated new ${type} challenge with difficulty ${difficulty}: ${savedChallenge.id}`,
    );

    return savedChallenge;
  }

  private getChallengeTemplate(
    type: ChallengeType,
    difficulty: DifficultyLevel,
  ): ChallengeTemplate {
    const templates = this.getChallengeTemplates();
    const typeTemplates = templates[type] || [];
    const difficultyTemplates = typeTemplates.filter(
      ...
      [ChallengeType.ALGORITHM]: [
        {
          ...
          content: {
            ...
            followUp:
              'Explain why QuickSort can degrade to O(n²) in the worst case.',
          },
          solution: {
            answer: 1,
            explanation:
