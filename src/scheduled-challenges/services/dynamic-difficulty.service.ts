...
import {
  ...
  AttemptStatus,
} from '../entities/challenge-attempt.entity';
import {
  type ChallengeType,
  DifficultyLevel,
...
export class DynamicDifficultyService {
  ...
  async updateUserDifficulty(
    ...
    profile.averageScore =
      (profile.averageScore * (profile.totalAttempts - 1) + attempt.score) /
      profile.totalAttempts;
    profile.averageTimeSpent =
      (profile.averageTimeSpent * (profile.totalAttempts - 1) +
        attempt.timeSpent) /
      profile.totalAttempts;
  ...
  private calculateNewDifficulty(
    ...
    const { successRate, totalAttempts, currentDifficulty, averageScore } =
      profile;
  ...
  async getUserDifficultyProfile(
    userId: string,
    challengeType: ChallengeType,
