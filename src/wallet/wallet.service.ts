...
export class WalletService {
  ...
  private currentConnectionStatus: WalletConnectionStatus = {
    isConnected: false,
  };
  ...
  async connect(providerName: string): Promise<WalletConnectionStatus> {
    ...
    try {
      ...
      this.logger.error(
        `Failed to connect to ${providerName}: ${error.message}`,
      );
  ...
  async disconnect(): Promise<void> {
    ...
    try {
      this.logger.log(
        `Attempting to disconnect from ${this.activeProviderName}...`,
      );
      ...
      this.logger.error(
        `Failed to disconnect from ${this.activeProviderName}: ${error.message}`,
      );
  ...
  async getAccounts(): Promise<string[]> {
    ...
    try {
      ...
      this.logger.error(
        `Failed to get accounts from ${this.activeProviderName}: ${error.message}`,
      );
      this.emitEvent<WalletErrorEvent>(WalletEvents.ERROR, {
        providerName: this.activeProviderName,
        error: { code: error.name, message: error.message },
  ...
  async getChainId(): Promise<string> {
    ...
    try {
      ...
      this.logger.error(
        `Failed to get chain ID from ${this.activeProviderName}: ${error.message}`,
      );
      this.emitEvent<WalletErrorEvent>(WalletEvents.ERROR, {
        providerName: this.activeProviderName,
        error: { code: error.name, message: error.message },
  ...
  async signMessage(message: string, address: string): Promise<Signature> {
    ...
    try {
      this.logger.log(
        `Attempting to sign message with ${this.activeProviderName} for ${address}...`,
      );
      const signature = await provider.signMessage(message, address);
      this.logger.log(
        `Message signed successfully by ${this.activeProviderName}.`,
      );
      return signature;
    } catch (error) {
      if (error instanceof UserRejectedTransactionException) {
        this.logger.warn(
          `User rejected message signing via ${this.activeProviderName}.`,
        );
        this.emitEvent<WalletErrorEvent>(WalletEvents.ERROR, {
          providerName: this.activeProviderName,
          address,
        ...
        this.logger.error(
          `Failed to sign message with ${this.activeProviderName}: ${error.message}`,
        );
        this.emitEvent<WalletErrorEvent>(WalletEvents.ERROR, {
          providerName: this.activeProviderName,
          address,
      ...
      try {
        ...
        if (
          ...
          try {
            ...
            this.currentConnectionStatus.chainId =
              transaction.chainId.toString();
            this.logger.log(
              `Successfully switched network to ${transaction.chainId}. Retrying transaction.`,
            );
        ...
        this.logger.log(
          `Transaction sent successfully by ${this.activeProviderName}: ${result.hash}`,
        );
        return result;
      } catch (error) {
        if (error instanceof UserRejectedTransactionException) {
          this.logger.warn(
            `User rejected transaction via ${this.activeProviderName}.`,
          );
  ...
  async switchNetwork(chainId: string): Promise<void> {
    ...
    try {
      this.logger.log(
        `Attempting to switch network to ${chainId} with ${this.activeProviderName}...`,
      );
      ...
      this.logger.log(
        `Successfully switched ${this.activeProviderName} to network ${chainId}.`,
      );
    } catch (error) {
      this.logger.error(
        `Failed to switch network with ${this.activeProviderName}: ${error.message}`,
      );
      this.emitEvent<WalletErrorEvent>(WalletEvents.ERROR, {
        providerName: this.activeProviderName,
        address: this.currentConnectionStatus.address,
  ...
  private emitEvent<T extends WalletEvent>(
    ...
    this.logger.debug(
      `Emitted event: ${eventName} with payload: ${JSON.stringify(fullPayload)}`,
    );
  }
