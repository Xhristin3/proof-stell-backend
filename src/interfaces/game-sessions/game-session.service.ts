...
export class GameSessionServiceImpl implements GameSessionService {
  ...
  async endSession(
    ...
    const integrityReport =
      await this.integrityService.validateSession(session);
  ...
  async recordAction(
    sessionId: string,
    actionData: Omit<GameAction, 'id' | 'serverTimestamp'>,
  ): Promise<GameAction> {
    const session = this.sessions.get(sessionId);
    if (!session) {
      throw new NotFoundException(`Session ${sessionId} not found`);
  ...
  async validateAction(
    ...
    const lastSequence = Math.max(
      ...(session.actions?.map((a) => a.sequence) || [0]),
    );
  ...
  private isValidActionTransition(
    fromAction?: string,
    toAction?: string,
  ): boolean {
