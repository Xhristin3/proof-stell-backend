...
export class SessionIntegrityService {
  ...
  private readonly secretKey =
    process.env.SESSION_SECRET_KEY || 'default-secret-key';
  ...
  async validateSession(
    ...
    try {
      ...
      this.logger.log(
        `Session ${session.id} integrity check completed: ${report.overallStatus}`,
      );
  ...
  private async validateTiming(
    ...
    if (
      duration < rules.timeThresholds.tooFast &&
      session.score &&
      session.score > 0
    ) {
      return {
        ...
        details: {
          duration,
          threshold: rules.timeThresholds.tooFast,
          score: session.score,
        },
      };
    }
  ...
  private async validateActionSequence(
    ...
    const missingActions = rules.requiredActions.filter(
      (required) => !actionTypes.has(required),
    );
    ...
    if (
      startAction &&
      completeAction &&
      startAction.timestamp > completeAction.timestamp
    ) {
      return {
        checkType: 'action_sequence',
        status: 'failed',
        ...
        timestamp: new Date(),
        details: {
          startTime: startAction.timestamp,
          completeTime: completeAction.timestamp,
        },
      };
    }
  ...
  private async validateRateLimit(
    ...
    for (let i = 0; i < actions.length - burstThreshold; i++) {
      ...
      const actionsInWindow = actions.filter(
        (a, idx) =>
          idx >= i && a.timestamp >= windowStart && a.timestamp <= windowEnd,
      ).length;
  ...
  private async logAnomalies(
    ...
    for (const anomaly of anomalies) {
      if (
        logLevel === 'all' ||
        (logLevel === 'warnings' && anomaly.severity !== 'low')
      ) {
