  ...
  describe('validateSession', () => {
    ...
    it('should detect impossible score', async () => {
      ...
      expect(
        report.checks.some(
          (check) =>
            check.checkType === 'score_validation' && check.status === 'failed',
        ),
      ).toBe(true);
      expect(
        report.anomalies.some(
          (anomaly) => anomaly.anomalyType === 'impossible_score',
        ),
      ).toBe(true);
    });
    ...
    it('should detect suspiciously fast completion', async () => {
      ...
      expect(
        report.checks.some(
          (check) =>
            check.checkType === 'time_validation' && check.status === 'warning',
        ),
      ).toBe(true);
    });
    ...
    it('should detect missing required actions', async () => {
      ...
      expect(
        report.checks.some(
          (check) =>
            check.checkType === 'action_sequence' && check.status === 'failed',
        ),
      ).toBe(true);
    });
    ...
    it('should detect rate limit violations', async () => {
      ...
      expect(
        report.checks.some(
          (check) =>
            check.checkType === 'rate_limiting' && check.status === 'failed',
        ),
      ).toBe(true);
    });
    ...
    it('should detect replay attacks', async () => {
      ...
      expect(
        report1.checks.some(
          (check) =>
            check.checkType === 'replay_detection' && check.status === 'passed',
        ),
      ).toBe(true);
      ...
      expect(
        report2.checks.some(
          (check) =>
            check.checkType === 'replay_detection' && check.status === 'failed',
        ),
      ).toBe(true);
    });
    ...
    it('should detect invalid metadata', async () => {
      ...
      expect(
        report.checks.some(
          (check) =>
            check.checkType === 'metadata_validation' &&
            check.status === 'failed',
        ),
      ).toBe(true);
    });
    ...
    it('should verify session signatures', async () => {
      ...
      expect(
        report.checks.some(
          (check) =>
            check.checkType === 'signature_verification' &&
            check.status === 'passed',
        ),
      ).toBe(true);
    });
    ...
    it('should detect tampered signatures', async () => {
      ...
      expect(
        report.anomalies.some(
          (anomaly) => anomaly.anomalyType === 'tampered_data',
        ),
      ).toBe(true);
    });
    ...
    it('should handle validation errors gracefully', async () => {
      ...
      expect(report.overallStatus).toBe('invalid');
      expect(report.recommendation).toBe('reject');
      expect(report.checks.some((check) => check.status === 'failed')).toBe(
        true,
      );
    });
  });
