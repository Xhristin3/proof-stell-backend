...
export class GameSessionController {
  ...
  async startSession(userId: string, dto: StartSessionDto) {
    try {
      const session = await this.gameSessionService.startSession(
        userId,
        dto.sessionType,
        dto.puzzleId,
        dto.moduleId,
  ...
  async endSession(sessionId: string, dto: EndSessionDto) {
    try {
      const integrityReport = await this.gameSessionService.endSession(
        sessionId,
        dto,
      );
  ...
  async getUserSessions(userId: string, limit?: number) {
    const sessions = await this.gameSessionService.getUserSessions(
      userId,
      limit,
    );
    return ResponseUtil.success(
  ...
  async validateSession(sessionId: string) {
    ...
    const integrityReport =
      await this.gameSessionService.validateSession(session);
    return ResponseUtil.success(
      integrityReport,
  ...
  async getSessionAnomalies(sessionId: string) {
    ...
    return ResponseUtil.success(
      anomalies,
      'Session anomalies retrieved successfully',
    );
  }

  @Get('users/:userId/anomalies')
  async getUserAnomalies(userId: string) {
    const anomalies = await this.gameSessionService.getAnomalies(
      undefined,
      userId,
    );
    return ResponseUtil.success(
      anomalies,
      'User anomalies retrieved successfully',
    );
  }
  ...
  async resolveAnomaly(anomalyId: string, dto: ResolveAnomalyDto) {
    try {
      await this.gameSessionService.resolveAnomaly(
        anomalyId,
        dto.moderatorNotes,
      );
  ...
  async getIntegrityStats() {
    ...
    return ResponseUtil.success(
      stats,
      'Integrity statistics retrieved successfully',
    );
  }
  ...
  async getSuspiciousSessions(limit = 50) {
    ...
    const allSessions = Array.from(
      (this.gameSessionService as any).sessions.values(),
    );
    const suspiciousSessions = allSessions
      .filter(
        (
          session: import('../../interfaces/game-session.interface').GameSession,
        ) => session.status === 'under_review' || session.status === 'invalid',
      )
      .slice(0, limit);

    return ResponseUtil.success(
      suspiciousSessions,
      'Suspicious sessions retrieved successfully',
    );
  }
