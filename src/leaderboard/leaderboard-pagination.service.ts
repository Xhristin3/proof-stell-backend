...
export class LeaderboardPaginationService {
    ...
    if (filters.scoreRange) {
      filtered = filtered.filter((entry) => {
        if (
          ...
          entry.score < filters.scoreRange.min
        ) {
          return false;
        }
        if (
          filters.scoreRange.max !== undefined &&
          entry.score > filters.scoreRange.max
        ) {
          return false;
        }
        return true;
  ...
  private getAppliedFilters(query: PaginatedLeaderboardQuery): string[] {
    ...
    if (query.challengeType)
      applied.push(`challengeType:${query.challengeType}`);
    if (query.timeframe && query.timeframe !== 'all_time')
      applied.push(`timeframe:${query.timeframe}`);
    if (query.search) applied.push(`search:${query.search}`);
    if (query.minScore !== undefined)
      applied.push(`minScore:${query.minScore}`);
    if (query.maxScore !== undefined)
      applied.push(`maxScore:${query.maxScore}`);
    if (query.sortBy && query.sortBy !== 'score')
      applied.push(`sortBy:${query.sortBy}`);
    if (query.sortOrder && query.sortOrder !== 'desc')
      applied.push(`sortOrder:${query.sortOrder}`);
  ...
  async getCursorPaginatedLeaderboard(
    ...
    if (cursor) {
      ...
      const cursorIndex = filteredEntries.findIndex(
        (entry) => this.encodeCursor(entry) === cursor,
      );
      if (cursorIndex !== -1) {
    ...
    const nextCursor =
      endIndex < filteredEntries.length
        ? this.encodeCursor(paginatedEntries[paginatedEntries.length - 1])
        : undefined;
    const prevCursor =
      startIndex > 0
        ? this.encodeCursor(filteredEntries[startIndex - 1])
        : undefined;
