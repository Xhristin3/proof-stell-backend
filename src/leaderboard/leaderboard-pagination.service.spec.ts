...
describe('LeaderboardPaginationService', () => {
  ...
  beforeEach(async () => {
    ...
    service = module.get<LeaderboardPaginationService>(
  ...
  describe('getPaginatedLeaderboard', () => {
    ...
    it('should filter by region', async () => {
      ...
      expect(result.data).toHaveLength(2);
      expect(result.data.every((entry) => entry.region === 'Europe')).toBe(
        true,
      );
    });
    ...
    it('should filter by score range', async () => {
      ...
      expect(
        result.data.every(
          (entry) => entry.score >= 2000 && entry.score <= 2500,
        ),
      ).toBe(true);
    });
    ...
    it('should handle challenge type filtering', async () => {
      ...
      expect(
        result.data.every((entry) => entry.totalPuzzlesCompleted >= 5),
      ).toBe(true);
    });
  ...
  describe('getCursorPaginatedLeaderboard', () => {
    it('should return first page without cursor', async () => {
      const result = await service.getCursorPaginatedLeaderboard(
        mockEntries,
        undefined,
        2,
      );
    ...
    it('should handle cursor-based pagination', async () => {
      // Get first page
      const firstPage = await service.getCursorPaginatedLeaderboard(
        mockEntries,
        undefined,
        2,
      );
      ...
      const secondPage = await service.getCursorPaginatedLeaderboard(
        mockEntries,
